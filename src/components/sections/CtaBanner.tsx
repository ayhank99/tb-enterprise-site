  ...
  if (variant === 'centered') {
    return (
      <section className="section-block bg-[color:var(--site-bg-soft)]">
        <Container>
          <article className="relative overflow-hidden rounded-[2.4rem] border border-[color:var(--site-border)] bg-white px-6 py-10 text-center shadow-[var(--site-card-shadow-hover)] md:px-10 md:py-14">
            <div className="pointer-events-none absolute -left-12 -top-12 h-40 w-40 rounded-full bg-[color:var(--site-primary-glow)]" />
            <div className="pointer-events-none absolute -bottom-12 -right-12 h-44 w-44 rounded-full bg-[color:var(--site-primary-soft)]" />

            <div className="relative mx-auto max-w-4xl">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[color:var(--site-primary)]">{cta.eyebrow}</p>
              <h2 className="mt-4 text-balance font-display text-3xl font-extrabold leading-tight tracking-[-0.05em] text-[color:var(--site-text)] md:text-5xl">
                {cta.title}
              </h2>
              <p className="mx-auto mt-5 max-w-3xl text-base leading-relaxed text-[color:var(--site-muted)]">{cta.text}</p>
